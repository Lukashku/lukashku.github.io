...
Now I know the files from the FTP can be directly accessed from the Ib page. I decide to check if I have write privileges to the FTP server so I could upload a reverse shell. I test this by upload a test file
...
```
If I go back to the Ib page and navigate to <b>test.txt</b> I see my file
...
```
Now in the Ib page, navigate to <b>http://10.10.10.5/shell.aspx</b>
...
meterpreter > getuid
Server username: IIS APPPOOL\Ib
```
...
[+] 10.10.10.5 - exploit/windows/local/ms16_016_Ibdav: The target service is running, but could not be validated.
